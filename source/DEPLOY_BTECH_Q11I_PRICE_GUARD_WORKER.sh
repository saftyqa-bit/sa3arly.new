...
(
  ...
  WORKER_EXPECTED="sa3arly-worker-ambiguityfix-113457"
  ...
  TAG="btechq11iguard"
  REV_SUFFIX="q11iguard-$(date -u +%H%M%S)"
  RELEASE_ID="q11iguard-$(date -u +%Y%m%d-%H%M%S)"
  WORK_DIR="$(mktemp -d)"
...
PY
  ...
  echo "=== BTECH Q11I SKU/PRICE GUARD PRECHECK ==="
...
' "$FAILED_TASK_ID" <<<"$RUN_BEFORE"
  ...
  grep -F 'version = "0.4.8"' pyproject.toml >/dev/null
  ...
  echo
  echo "=== BUILDING BTECH-Q11I-SKU-PRICE-GUARDED WORKER IMAGE ==="
...
'
  ...
  echo
  echo "=== SWITCHING TO BTECH-Q11I-SKU-PRICE-GUARDED WORKER ==="
...
' <<<"$RUN_AFTER"
  ...
  echo "BTECH_Q11I_PRICE_GUARD_DEPLOYED=SUCCESS_NO_TASK_EXECUTED"
)

STEP_RC=$?

echo
echo "BTECH_Q11I_PRICE_GUARD_DEPLOY_SCRIPT_EXIT=${STEP_RC}"
exit "$STEP_RC"
